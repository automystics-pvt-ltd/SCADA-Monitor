...
export default function ReportCenter({ siteName, sites, devices, parameters }: { siteName: string; sites: string[]; devices: Array<{ id: string; name: string; site: string; type: string }>; parameters: string[] }) {
  ...
  const exportPdf = () => {
    ...
    void runExport('pdf', (completeResult) => {
      const rows = completeResult.records.map((record) => `<tr><td>${xml(record.category)}</td><td>${xml(record.deviceName ?? record.deviceId ?? '—')}</td><td>${xml(record.displayLabel)}</td><td>${record.value === null ? '—' : xml(record.value)}</td><td>${xml(record.unit)}</td><td>${xml(record.sourceReportedValue ?? '—')}</td><td>${xml(record.sourceReportedUnit ?? '—')}</td><td>${xml(record.transportRawValue ?? '—')}</td><td>${xml(record.sourceName)}</td><td>${xml(record.observedAt)}</td><td>${xml(record.provenance)}</td><td>${xml(record.quality)}</td></tr>`).join('');
      printWindow.document.write(`<!doctype html><html><head><title>${xml(completeResult.title)}</title><style>body{font:12px Arial;color:#172033;padding:24px}h1{margin:0 0 6px}p{color:#475569}table{width:100%;border-collapse:collapse;margin-top:20px}th,td{border:1px solid #cbd5e1;padding:7px;text-align:left;vertical-align:top}th{background:#eff6ff;font-size:10px;text-transform:uppercase}footer{margin-top:20px;color:#64748b}@media print{body{padding:0}}</style></head><body><h1>${xml(completeResult.title)}</h1><p><strong>Site:</strong> ${xml(completeResult.siteName)} &nbsp; <strong>Period:</strong> ${xml(completeResult.period.label)}<br/><strong>Generated:</strong> ${xml(formatDateTime(completeResult.generatedAt))}<br/>${xml(completeResult.sourceStatus)}</p><table><thead><tr><th>Category</th><th>Device</th><th>Parameter</th><th>Validated value</th><th>Validated unit</th><th>Source-reported value</th><th>Source unit</th><th>Transport raw</th><th>Source</th><th>Observed</th><th>Provenance</th><th>Quality</th></tr></thead><tbody>${rows || '<tr><td colspan="12">No included source records for the selected filters.</td></tr>'}</tbody></table><footer>${xml(completeResult.attribution)}</footer><script>window.onload=()=>window.print()</script></body></html>`);
