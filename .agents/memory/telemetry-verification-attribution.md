...
**How to apply:** Accept stable source identifiers as testable devices. Attribute payloads without a site label to a managed site only when exactly one active managed site exists; preserve explicit payload labels and reject ambiguous multi-site attribution. A recent, direct, non-retained message inside the live freshness window is valid test evidence when the publishing cadence is longer than the observation window.